...
VKAPI_ATTR VkBool32 VKAPI_CALL debug_message_callback(
    ...
    void * /*pUserData*/) {
  auto cstr_to_string = [](const char *c) {
    return (c == nullptr) ? "Null" : std::string_view(c);
  };
  ...
  fmt::format_to(
      ...
      cstr_to_string(pCallbackData->pMessageIdName),
      pCallbackData->messageIdNumber, cstr_to_string(pCallbackData->pMessage));
  ...
  if (0 < pCallbackData->cmdBufLabelCount) {
    ...
    for (uint32_t i = 0; i < pCallbackData->cmdBufLabelCount; i++) {
      fmt::format_to(
          std::back_inserter(buffer), "\t\tlabelName = <{}>\n",
...
void VkContext::CreateInstance() {
  ...
  system_info_.available_extensions_ =
...
bool VkContext::CheckLayerSupport(
    const std::vector<const char *> &layers_require) {
...
void VkContext::CreatePhysicalDevice() {
  ...
  if constexpr (axes::utils::get_platform_type() ==
                utils::PlatformType::kApple) {
    if (auto it =
            std::find_if(e.begin(), e.end(),
                         [](const char *ext) {
                           return strcmp(ext, "VK_KHR_portability_subset") == 0;
                         });
        it == e.end()) {
  ...
  if (!create_info_.headless_mode_) {
    ...
    if (auto it = std::find_if(
            e.begin(), e.end(),
            [](const char *ext) {
              return strcmp(ext, VK_KHR_SWAPCHAIN_EXTENSION_NAME) == 0;
            });
        it == e.end()) {
...
SystemInfo::PhysicalDeviceInfo
VkContext::GetDeviceInfo(vk::PhysicalDevice device) const {
