...
#include "axes/gui/imnodes/imnodes_int.hpp"
...
inline bool RectangleOverlapsBezier(const ImRect& rectangle, const CubicBezier& cubic_bezier) {
  ...
  for (int s = 0; s < cubic_bezier.NumSegments; ++s) {
    ImVec2 next = EvalCubicBezier(static_cast<float>((s + 1) * dt), cubic_bezier.P0,
...
void ImDrawListSplitterSwapChannels(ImDrawListSplitter& splitter, const int lhs_idx,
                                    const int rhs_idx) {
  if (lhs_idx == rhs_idx) {
    return;
  }
...

  {
    ...
    if ((GImNodes->Style.Flags & ImNodesStyleFlags_NodeOutline) != 0) {
      ...
      GImNodes->CanvasDrawList->AddRect(
          node.Rect.Min, node.Rect.Max, node.ColorStyle.Outline, node.LayoutStyle.CornerRounding,
          ImDrawFlags_RoundCornersAll, node.LayoutStyle.BorderThickness);
#endif
    }
  }
...
template <typename T> void ClearObjectSelection(const ImObjectPool<T>& objects,
...
template <typename T> bool IsObjectSelected(const ImObjectPool<T>& objects,
                                            ImVector<int>& selected_indices, const int id) {
  const int idx = ObjectPoolFind(objects, id);
...
bool IsLinkCreated(int* started_at_node_id, int* started_at_pin_id, int* ended_at_node_id,
                   int* ended_at_pin_id, bool* created_from_snap) {
  IM_ASSERT(GImNodes->CurrentScope == ImNodesScope_None);
...
const char* SaveEditorStateToIniString(const ImNodesEditorContext* const editor_ptr,
