...
#define AX_OMP_CRITICAL() AX_PRAGMA(omp critical(name))
...
#define AX_OMP_PARALLEL_FOR_SCHEDULE_DYNAMIC() AX_PRAGMA(omp parallel for schedule(dynamic))
#define AX_OMP_PARALLEL_FOR_SCHEDULE_STATIC(size) AX_PRAGMA(omp parallel for schedule(static, size))
#define AX_OMP_PARALLEL_FOR_SCHEDULE_GUIDED() AX_PRAGMA(omp parallel for)
