...
public:
   ...
   */
  void
  RunTransientCommandInplace(const std::function<void(vk::CommandBuffer)> &f,
                             vk::Queue q);

  VmaAllocBuffer AllocateBuffer(vk::BufferCreateInfo buf_info,
  ...
  VmaAllocImage AllocateImage(vk::ImageCreateInfo img_info,
                              VmaAllocationCreateInfo info);
...
private:
  ...
  bool
  CheckExtensionSupport(const std::vector<const char *> &extensions_require);
