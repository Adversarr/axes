...
template<idx dim>
static math::matr<dim+1, dim+1> p1_e(
  const elements::P1Element<dim> E,
...
template<idx dim>
static math::sp_coeff_list p1(
  MeshBase<dim> const& mesh_,
  real uniform_density
) {
...
template<idx dim>
math::sp_coeff_list MassMatrixCompute<dim>::operator()(real density) {
...
template <idx dim>
math::sp_coeff_list MassMatrixCompute<dim>::operator()(
  math::field1r const& density,
  bool is_density_on_element
) {
  if (mesh_->GetType() == MeshType::kP1) {
