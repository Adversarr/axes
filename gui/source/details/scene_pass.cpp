...
vk::CommandBuffer SceneRenderPass::Draw() {
  ...
  std::array<vk::ClearValue, 2> clear_value = {background_color_,
                                               depth_stencil_value_};
...
// void SceneRenderPass::UpdateUniformBuffer() {

// }

void SceneRenderPass::AddPipeline(std::unique_ptr<ScenePipelineBase>&& pipeline) {
  pipelines_.push_back(std::move(pipeline));
}

vk::RenderPass SceneRenderPass::GetRenderPass() {
  return render_pass_;
}
