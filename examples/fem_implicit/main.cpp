...
void update_entity() {
  ...
  auto energy = deform.EnergyToVertices(elast.Energy(math::vec2r{1, 1}));
  real m = energy.minCoeff(), M = energy.maxCoeff();
  gl::Colormap mapping(0, 3e-4);
  AX_LOG(INFO) << "m=" << m << "\tM=" << M;
