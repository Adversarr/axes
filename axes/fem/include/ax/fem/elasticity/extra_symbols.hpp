...
template <typename Derived> auto green_strain(math::MBcr<Derived> F) {
  return 0.5 * (F.transpose() * F - math::eye<math::rows_v<Derived>>());
}

template <typename Derived> auto approx_green_strain(math::MBcr<Derived> F) {
  return 0.5 * (F.transpose() + F) - math::eye<math::rows_v<Derived>>();
}
...
AX_FORCE_INLINE void add_HJ(math::matr<4, 4>& H, const math::matr<2, 2>& /*F*/, real scale) {
  H(3, 0) += scale;
  H(0, 3) += scale;
  H(1, 2) -= scale;
  H(2, 1) -= scale;
...
template <typename Derived> AX_FORCE_INLINE math::mat3r x_hat(math::MBcr<Derived> Fi) {
  math::mat3r x_hat;
  x_hat << 0, -Fi(2), Fi(1), Fi(2), 0, -Fi(0), -Fi(1), Fi(0), 0;
  return x_hat;
}

AX_FORCE_INLINE void add_HJ(math::matr<9, 9>& H, const math::matr<3, 3>& F, real scale) {
