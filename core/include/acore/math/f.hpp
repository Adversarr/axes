#pragma once
#include "details/f-inl.hpp"

namespace axes::math {

/**
 * Return a constant value of type `T`, replacement for `static_cast<...>(
 * SOME_LITERAL );
 * @tparam T
 * @param val
 * @return
...
template <int times, typename T> constexpr auto pow(T value) noexcept {
  ...
  return details::Func<T>{}.template Pow<times>(value);
}
...
template <typename T> constexpr auto sqrt(T &&value) {
  return details::Func<std::decay<T>>{}.Sqrt(value);
}
