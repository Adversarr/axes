...
#include "axes/fem/elements.hpp"

namespace ax::fem {

constexpr real p13_element_f_f[3][3] = {{1.0 / 12, 1.0 / 24, 1.0 / 24},
                                        {1.0 / 24, 1.0 / 12, 1.0 / 24},
                                        {1.0 / 24, 1.0 / 24, 1.0 / 12}};

// [K][i][j] = Integrate [(diff phi_i) / (diff x_K), phi_j]
constexpr real p13_element_pfpx_f[3][3][2]
...
struct P1Element2D {
  ...
  AX_FORCE_INLINE P1Element2D(std::array<math::vec2r, 3> const& nodes) {
    ...
    det_jacobi_ = jacobi_.determinant();
  }

  AX_FORCE_INLINE real Integrate_F_F(idx i, idx j) const noexcept { return det_jacobi_ * p13_element_f_f[i][j]; }

  AX_FORCE_INLINE real Integrate_PF_F(idx i, idx j, idx i_pf) const noexcept {
    real pfi_pu = p13_element_pfpx_f[0][i_pf][j];
    real pfi_pv = p13_element_pfpx_f[1][i_pf][j];
