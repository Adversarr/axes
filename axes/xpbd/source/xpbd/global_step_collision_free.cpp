...
void global_step_collision_free(math::field3r const& weighted_position, math::field1r const& weights) {
  ...
  for (auto i: utils::iota(nV)) {
    for (auto [j, f]: utils::enumerate(g.faces_)) {
      ...
      if (auto collision = detect_vertex_face(vi_prev, vi_next, fj_prev, fj_next, 1e-5)) {
        ...
        AX_LOG(ERROR) << "Collision detected: " << collision.rel_t_ << " at vertex " << i << " and face " << j;
  ...
  // update the position.
  for (auto i: utils::iota(nV)) {
    expect_position.col(i) = math::lerp(g.last_vertices_.col(i), expect_position.col(i), min_toi(i));
  }
