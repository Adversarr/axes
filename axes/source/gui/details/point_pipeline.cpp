#include "axes/gui/details/point_pipeline.hpp"
...
PointPipeline::PointPipeline(std::shared_ptr<VkContext> vkc,
...
void PointPipeline::CreatePipeline(vk::RenderPass pass) {
  vk::ShaderModule vert_module, frag_module;
  {
    auto code = utils::io::read_binary(SPV_HOME "point.vert.spv");
    vk::ShaderModuleCreateInfo info;
    info.setPCode(reinterpret_cast<uint32_t *>(code.data()))
        .setCodeSize(code.size());
    vert_module = vkc_->GetDevice().createShaderModule(info);
    code = utils::io::read_binary(SPV_HOME "point.frag.spv");
    info.setPCode(reinterpret_cast<uint32_t *>(code.data()))
        .setCodeSize(code.size());
...
void PointPipeline::CreateEntityRenderData(ecs::EntityID ent,
  ...
  // Foreach point
  ecs::ComponentManager<PointRenderDataGpu> cm;
  PointRenderDataGpu *mrd = cm.AttachOrGet(ent, vkc_);
  // Test if the point rendering is valid.
...
void PointPipeline::Draw(vk::CommandBuffer &cbuffer) {
  // Bind Pipeline
  ...
  ecs::ComponentManager<PointRenderDataGpu> manager;
  for (auto [ent, render_data] : manager) {
    ...
    cbuffer.pushConstants(
        ...
        sizeof(PointPushConstants), &render_data->pc_);
    // cbuffer.drawIndexed(render_data->index_count_, render_data->instance_count_, 0,
    //                     0, 0);
    cbuffer.draw(shared->vertex_count_, 1, 0, 0);
  }
}
