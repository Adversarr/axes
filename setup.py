...
enable_cuda = 'ON' if this_platform != 'darwin' else 'OFF'
...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        cmake_args += [
            f'-DAX_ENABLE_CUDA={enable_cuda}',       # Enable CUDA
            '-DAX_ENABLE_PYBIND=ON',
        ]
        ...
        subprocess.run(
            ["cmake", "--build", ".", *build_args, "--target", "pyax"], cwd=build_temp, check=True
        )
...
setup(
    ...
    version="0.0.1",
    author="Adversarr",
    author_email="xxx@xxx.cn",
    description="Python binding for axes",
    long_description="",
    ext_modules=[CMakeExtension("pyax", ".")],
