...
#include "axes/core/math/convention.hpp"
...
struct CoordinateTransformBase {
  ...
  template <typename... Args,
            typename
            = std::enable_if_t<std::is_constructible_v<Domain, Args...>>>
...
template <typename Scalar, int dim> struct BiasTransform
    : public CoordinateTransformBase<BiasTransform<Scalar, dim>,
                                     Vector<Scalar, dim>, Vector<Scalar, dim>> {
  using domain_type = Vector<Scalar, dim>;
  using range_type = Vector<Scalar, dim>;
 ...
 * Assuming the lower bound is [0, UB[I]] ...
 *
 * @tparam dim
 */
template <int dim> struct DiscreteStorageSequentialTransform
    : public CoordinateTransformBase<DiscreteStorageSequentialTransform<dim>,
                                     IndexVector<dim>, Index> {
  using domain_type = IndexVector<dim>;
  using range_type = size_t;

  DiscreteStorageSequentialTransform(const DiscreteStorageSequentialTransform &)
      = default;
  ...
  NdRange<dim> Iterate() const noexcept {
    return NdRange<dim>(make_array(bound_));
  }

  size_t Size() const noexcept {
    return static_cast<size_t>(multiplier_.array().prod());
  }
...
auto combine_transform(Front &&f, Back &&b) {
  return CombinedTransform<std::remove_cvref_t<Front>,
                           std::remove_cvref_t<Back>>(std::forward<Front>(f),
                                                      std::forward<Back>(b));
}
