...
struct Graph::Impl {
  ...
  void EmplaceNode(UPtr<NodeBase> n) {
    ...
    if (uinfo.real_id_ == invalid_id) {
      ...
      AX_DCHECK(nodes_[n->id_].get() == nullptr) << "You are trying to overwrite an existing node!";
  ...
  void RemoveNode(idx i) {
    auto& n = nodes_[i];
    AX_DCHECK(i == n->id_) << "Internal Logic Error!";
    n.reset(nullptr);
    UuidRecycle(i);
  }
  ...
  void EmplaceSocket(Socket s) {
    ...
    if (uinfo.real_id_ == invalid_id) {
      ...
      AX_DCHECK(sockets_[s.id_].id_ == s.id_) << "You are trying to overwrite an existing socket!";
  ...
  void RemoveSocket(idx i) {
    auto& s = sockets_[i];
    AX_DCHECK(i == s.id_) << "Internal Logic Error!";
    s.id_ = invalid_id;
    UuidRecycle(i);
  }
...
NodeBase const* Graph::GetNode(idx id) const {
  return const_cast<Graph*>(this)->GetNode(id);
}

bool Graph::RemoveNode(NodeBase* n) {
  return RemoveNode(n->id_);
}
...
Socket* Graph::AddSocket(idx left, idx input_pin, idx right, idx output_pin) {
  ...
  return AddSocket(&(data_in_node->outputs_[input_pin]), 
...
Socket* Graph::GetSocket(idx input_node, idx input_pin, idx output_node, idx output_pin) {
  ...
  return GetSocket(data_in_node->outputs_[input_pin].id_, 
                   data_out_node->inputs_[output_pin].id_);
}

bool Graph::RemoveSocket(idx id) {
  return RemoveSocket(GetSocket(id));
}
...
Pin* Graph::GetPin(idx id) {
  ...
  return pin_to_node_info.is_input_ ? &(node->inputs_[pi]) : &(node->outputs_[pi]);
}

Pin const* Graph::GetPin(idx id) const {
  return const_cast<Graph*>(this)->GetPin(id);
}
...
idx Graph::GetNumNodes() const {
  return (idx) impl_->nodes_.size();
}

idx Graph::GetNumSockets() const {
  return (idx) impl_->sockets_.size();
}

idx Graph::GetCurrentUuid() const {
  return impl_->uuid_next_;
}
