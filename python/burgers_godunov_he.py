...
def plot_err_nx(Nx_list, l_inf_list, l2_list, T, prefix):
  plt.figure(dpi=300)
  plt.title('Error v.s. $N_x$')
  plt.loglog(Nx_list, l_inf_list, label='$l_\\infty$', marker='o')
  plt.loglog(Nx_list, l2_list, label='$l_1$', marker='o')

  # compute the order:
  for i in range(1, len(Nx_list)):
    l_inf_increased = l_inf_list[i] / l_inf_list[i - 1]
    l2_increased = l2_list[i] / l2_list[i - 1]
    Nx_increased = 2.0
    l_inf_order = -np.log(l_inf_increased) / np.log(Nx_increased)
    l2_order = -np.log(l2_increased) / np.log(Nx_increased)
    print(f'[{prefix}]: Nx={Nx_list[i-1]} to Nx={Nx_list[i]}: l_inf_order={l_inf_order}, l2_order={l2_order}')
...
plot_err_nx(Nx_list, l_inf_list, l2_list, T, 't05')

T = 1.5
# Nx_list = [20, 80]

super_res = 640 * 8
subprocess.call([accurate, f'--alpha=0.5', f'--nx={Nx+1}', '--export', 
                 f'--export_file=exact_{Nx}_{int(10*T)}.npy', f'--t={T}'])
  ...
  for l, lname in enumerate(LIMITOR_NAME):
    for m in (M if l == 2 else [0.1]):
      ...
      for Nx in Nx_list:
        ...
        plt.close()
      plot_err_nx(Nx_list, l_inf_list, l2_list, T, f'l{l}m{int(m*10)}f{flux}')
