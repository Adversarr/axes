...
#include <agui/details/ui_pass.hpp>
#include <agui/details/vkcontext.hpp>
#include <agui/details/vkgraphics.hpp>
...
int main() {
  ...
  gui::InstanceInfo iinfo;
  iinfo.position_.setZero();
  iinfo.rotation_.setZero();
  data.instances_.Resize(1);
  data.instances_[0] = iinfo;
...

  {
    ...
    auto *index_arr = static_cast<uint32_t *>(
        index_buffer.alloc_info_.pMappedData);
    ...
    auto *prender = em.Attach<gui::details::MeshRenderDataGpu>();
    ...
    auto *pshared = em.Attach<gui::details::SceneRenderDataSharedGpu>();
    ...
    auto mp = std::make_unique<gui::details::MeshPipeline>(vkc, vkg, sc.get());
    ...
    vkc->FreeBuffer(vertex_buffer);
    vkc->FreeBuffer(index_buffer);
    vkc->FreeBuffer(instance_buffer);
  }
  shutdown();
  return 0;
}
