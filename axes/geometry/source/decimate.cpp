...
void MeshDecimator::Run() {
  std::map<HalfedgeVertex*, math::RealMatrix4> Q_i;
  mesh_->ForeachVertex([&](HalfedgeVertex* vert) {
    math::RealMatrix4 m = math::RealMatrix4::Zero();
    mesh_->ForeachEdgeAroundVertex(vert, [&](HalfedgeEdge* edge) {
      auto normal = edge->Normal().normalized();
      auto abcd = math::RealVector4{normal.x(), normal.y(), normal.z(), -vert->position_.dot(normal)};
  ...
  while (mesh_->NVertices() > target_count_ && !cost.empty()) {
    ...
    math::RealVector3 target_position = min_cost.target_position;
    ...
    math::RealMatrix4 Q_head = math::zeros<4, 4>();
    mesh_->ForeachEdgeAroundVertex(collapse_vertex, [&](HalfedgeEdge* e) {
      auto normal = e->Normal().normalized();
      auto abcd = math::RealVector4{normal.x(), normal.y(), normal.z(),
    ...
    mesh_->ForeachEdgeAroundVertex(collapse_vertex, [&](HalfedgeEdge* e) {
      ...
      math::RealMatrix4 Q_v = math::zeros<4, 4>();
      mesh_->ForeachEdgeAroundVertex(v, [&Q_v, &v](HalfedgeEdge* e2) {
        math::RealVector3 normal = e2->Normal().normalized();
        auto abcd = math::RealVector4{normal.x(), normal.y(), normal.z(), -v->position_.dot(normal)};
