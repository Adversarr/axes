...
template <int dim>
math::RealSparseMatrix LaplaceMatrixCompute<dim>::operator()(Real W) {
  ...
  for (Index i = 0; i < nE; ++i) {
    ...
    local = W * eigen_vectors * eigen_values.asDiagonal() * eigen_vectors.transpose();
    for (Index i = 0; i <= dim; ++i) {
      for (Index j = 0; j <= dim; ++j) {
        l_coef.emplace_back(static_cast<math::SparseIndex>(elem[i]),
                            static_cast<math::SparseIndex>(elem[j]), local(i, j));
      }
    }
  }
...
template <int dim>
math::RealSparseMatrix LaplaceMatrixCompute<dim>::operator()(math::RealField1 const& W) {
  math::RealSparseCOO l_coef;
  auto const total = static_cast<size_t>(mesh_.GetNumElements() * (dim + 1) * (dim + 1));
  l_coef.reserve(total);
  Index nE = mesh_.GetNumElements();
  for (Index iElem = 0; iElem < nE; ++iElem) {
    ...
    auto [eigen_vectors, eigen_values] = math::eig(local);
    eigen_values = eigen_values.cwiseMax(0);
    local = W(iElem) * eigen_vectors * eigen_values.asDiagonal() * eigen_vectors.transpose();
    for (Index i = 0; i <= dim; ++i) {
      for (Index j = 0; j <= dim; ++j) {
        l_coef.push_back({static_cast<int>(elem[i]), static_cast<int>(elem[j]), local(i, j)});
      }
    }
  }
  Index dofs = mesh_.GetNumVertices();
